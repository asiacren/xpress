<template>
  <div class="video-player">
    <video autoplay="autoplay" class="video-js vjs-custom-skin" :class="{ 'live': options.live }"></video>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      initialize: function() {
        ...
        playsinline && (this.$el.children[0].setAttribute('webkit-playsinline', playsinline), this.$el.children[0].setAttribute('playsinline', playsinline))
        ...
        this.player = videojs(this.$el.children[0], video_options, function() {
          ...
          this.on('timeupdate', function() {
            _this.$emit && _this.$emit(customEventName, {
              timeupdate: true,
              currentTime: this.currentTime() })
            _this.$dispatch && _this.$dispatch(customEventName, {
              timeupdate: true,
              currentTime: this.currentTime() })
          })
          // 监听error
          this.on('error', function() {
            _this.$emit && _this.$emit(customEventName, { error: true })
            _this.$dispatch && _this.$dispatch(customEventName, { error: true })
          })
          // 监听abort
          this.on('abort', function() {
            _this.$emit && _this.$emit(customEventName, { abort: true })
            _this.$dispatch && _this.$dispatch(customEventName, { abort: true })
          })
          this.on('loadstart', function() {
            _this.$emit && _this.$emit(customEventName, { loadstart: true })
            _this.$dispatch && _this.$dispatch(customEventName, { loadstart: true })
          })
          this.on('progress', function() {
            _this.$emit && _this.$emit(customEventName, { progress: true })
            _this.$dispatch && _this.$dispatch(customEventName, { progress: true })
          })
